...
In this laboratory exercise, we will create a simple application that blinks LEDs connected to the Microblaze MCS core. The LEDs will be turned on and off every second. In addition, the application will print a message about the state of SWITCHES to the console.

Note: 
- In this example, we will enable the GPIO peripheral in the Microblaze MCS core to control the LED. The GPIOS peripheral is a general-purpose input/output peripheral that can be used to control LEDs, switches, and other devices.
- In addition to the GPIO peripheral, we will enable the UART peripheral to print messages to the console.
- To enable delay in the application, we will use the `sleep` function from the `sleep.h` library.